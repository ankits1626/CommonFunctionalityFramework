//
//  FeedbackCommentChatBarView.swift
//  CommonFunctionalityFramework
//
//  Created by Suyesh Kandpal on 11/08/23.
...
public class FeedbackCommentChatBarViewError {
    static let NoHeightConstraintSet : Error = NSError(
        domain: "com.cff.FeedbackCommentChatBarView",
        code: 1,
        userInfo: [NSLocalizedDescriptionKey: "Height constraint is not set for chat bar."]
    )
}
@objc public protocol FeedbackCommentChatBarDelegate : class {
    func finishedPresentingOverKeyboard()
    func addAttachmentButtonPressed()
    func removeAttachment()
    func rightButtonPressed(_ chatBar : FeedbackCommentChatBarView)
}

public class FeedbackCommentChatBarView : UIView {
    ...
    @IBOutlet public weak var delegate : FeedbackCommentChatBarDelegate?
...
//        superview?.addGestureRecognizer(tap)
    ...
    func disable() throws {
        if let constraint = heightConstraint{
            ...
            throw FeedbackCommentChatBarViewError.NoHeightConstraintSet
        }
    }
...
extension FeedbackCommentChatBarView{
...
extension FeedbackCommentChatBarView : ASMentionCoordinatortextUpdateListener{
    func textUpdated() {
        taggedMessaged = ASMentionCoordinator.shared.getPostableTaggedText() ?? ""
    }
}

extension FeedbackCommentChatBarView : AttachmentHandlerDelegate{
